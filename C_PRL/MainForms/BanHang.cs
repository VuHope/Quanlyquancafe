...
namespace C_PRL.MainForms
{
    public partial class BanHang : Form
    {
        ...
        public void LoadSp()
        {
            ...
            if (_sanphamServices.GetAllSanpham().Where(sp => sp.Trangthai != "Không bán") != null)
            {
                foreach (var item in _sanphamServices.GetAllSanpham().Where(sp => sp.Trangthai != "Không bán"))
                {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (validate())
            {
                ...
                if (cb_makm.Items.Count > 0)
                {
                    if (!QL_NhanVien.Validatecombobox(cb_makm.SelectedItem.ToString(), cb_makm, "Mã khuyến mãi"))
                    {
                        return;
                    }
                }
        ...
        {
            Ban_Hoadon ban_hoadon = new Ban_Hoadon();
            ban_hoadon.BackColor = Color.MistyRose;
            ban_hoadon.Show();
        }
    }
}

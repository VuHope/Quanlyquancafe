...
namespace C_PRL.MainForms
{
    public partial class QL_Sanpham : Form
    {
        ...
        private void btn_themLoaisp_Click(object sender, EventArgs e)
        {
            ...
            loaiSanPham.BackColor = Color.MistyRose;
        ...
        public bool IsNulOrEmty(string masp, string tensp, string giaban, object trangthai)
        {
            ...
            else if (!Regex.IsMatch(masp, @"^[A-Za-z0-9]{1,10}$"))
            {
        ...
        private void btn_themSp_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!QL_NhanVien.Validatecombobox(cb_Trangthai.Text, cb_Trangthai, "Trạng thái"))
                {
                    return;
                }
        ...
        private void dgv_sanpham_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1)
            {
                ...
                txt_giaban.Text = ((int)Math.Round((decimal)dgv_sanpham.Rows[e.RowIndex].Cells[4].Value)).ToString();
        ...
        private void btn_timKiem_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(txt_tkMasp.Text) && Regex.IsMatch(txt_tkMasp.Text, @"^[A-Za-z0-9]{1,10}$"))
            {
            ...
            if (cb_tkLoaisp.SelectedValue == null )
            {
                ...
                if (cb_tkLoaisp.Items.Count > 0)
                {
                    foreach (var item in cb_tkLoaisp.Items)
                    {
                        if (cb_tkLoaisp.Text == item.ToString())
                        {
                            loaisp = "";
                        }
                    }
                    loaisp = $" Maloaisp = N'{cb_tkLoaisp.SelectedValue}' and ";
                } 

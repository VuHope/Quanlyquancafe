...
namespace C_PRL.MainForms
{
    public partial class QL_CaLam : Form
    {
        ...
        {
            InitializeComponent();
            dgv_bangLuong.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
            dgv_calam.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
        }
        ...
        public void loadCaLam()
        {
            var calam = (from cl in _caLamServices.GetAllCalam()
                         ...
                         select new
                         {
                             ...
                             Giobd = cl.Giobd,
                             Giokt = cl.Giokt,
                             Ngaylamviec = cl.Ngaylamviec.ToString("dd-MM-yyyy"),
                             Manv = cl.Manv,
                         }
                         ).ToList();
        ...
        private void btn_cthem_Click(object sender, EventArgs e)
        {
            if (isNullOrEmty())
            {
                if (!Cb_valitdate(cb_cManv.Text, cb_cManv))
                {
                    return;
                }
        ...
        private void QL_CaLam_Load(object sender, EventArgs e)
        {
            ...
            dtp_ngayLamViec.CustomFormat = "dd-MM-yyyy";
        ...
        private void dgv_calam_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1)
            {
                ...
                dtp_ngayLamViec.Text = DateTime.Parse(dgv_calam.Rows[e.RowIndex].Cells[3].Value.ToString()).ToString("dd-MM-yyyy");
        ...
        public void LoadLuong()
        {
            var dataLuong = (from l in _luongServices.GetAllLuong()
                             select new
                             {
                                 ...
                                 Manv = l.Manv,
                                 Sotien = l.Sotien,
                                 Ngaytra = l.Ngaytra?.ToString("dd-MM-yyyy"),
                             }).ToList();
        ...
        public bool Cb_valitdate(string manv, System.Windows.Forms.ComboBox comboBox)
        {
            if (comboBox.Items.Count > 0)
            {
                bool x = false;
                foreach (var item in _nhanvienServices.GetNhanvienList())
                {
                    if (manv == item.Manv.ToString())
                    {
                        x = true;
                    }
                }
        ...
        {
            if (!Cb_valitdate(cb_lManv.Text, cb_lManv))
            {
                return;
            }
        ...
        private void dgv_bangLuong_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1)
            {
                ...
                dtp_Ngaytra.Text = DateTime.Parse(dgv_bangLuong.Rows[e.RowIndex].Cells[3].Value.ToString()).ToString("dd-MM-yyyy");
            }
        }

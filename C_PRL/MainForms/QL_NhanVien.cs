...
namespace C_PRL.MainForms
{
    public partial class QL_NhanVien : Form
    {
        ...
        public bool IsNullOrEmty()
        {
            ...
            if (cb_chucvu.Items.Count > 0)
            {
                bool x = false;
                foreach (var item in _chucvuServices.GetAllChucvu())
                {
                    if (cb_chucvu.Text == item.Tenchucvu)
                    {
                        x = true;
                    }
                }
                if (!x)
                {
                    MessageBox.Show("Chức vụ không hợp lệ", "Error");
                    return false;
                }
            }
            return true;
        ...
        static public bool Validatecombobox(string input, ComboBox Combox, string NameOfCombox)
        {
            if (Combox.Items.Count > 0)
            {
                foreach (var item in Combox.Items)
                {
                    if (input == item.ToString())
                    {
                        return true;
                    }
                } 
            }
            MessageBox.Show($"{NameOfCombox} có giá trị không hợp lệ");
            return false;
        }
        ...
        private void btn_timkiem_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txt_Manv.Text) && !Regex.IsMatch(txt_Manv.Text, @"^[A-Za-z0-9]{1,10}$"))
            {
                manv = "";
            }
            ...
            else chucvu = $" chucvu = '{cb_chucvu.SelectedValue.ToString()}' and ";
            ...
            else taikhoan = $" taikhoan = '{txt_Taikhoan.Text}' and ";

            if (string.IsNullOrWhiteSpace(txt_Matkhau.Text))
            {
                matkhau = "";
            }
            else matkhau = $" matkhau = '{txt_Matkhau.Text}' and ";
